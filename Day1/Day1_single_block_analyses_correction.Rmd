...
### Plot the explained variances #FIX this 

-   scree plot
-   Note about pcaVarVn vs modelDF@R2X  
    If you want the classic PCA-style scree plot → you should use pcaVarVn. That’s the eigenvalue-based variance explained per component, and that’s what most people expect when they see a scree plot: each bar/point = % variance captured by that PC. If you want to reflect what the actual ropls model says it explains → you could use modelDF@R2X, but then it’s not a scree plot in the traditional sense; it’s more like a cumulative variance plot from the model’s perspective.
...
variances_genes_R2X <- data.frame(variance = PCA_genes_res@modelDF$R2X)
variances_genes$Dim <- rownames(variances_genes)
variances_genes_R2X$Dim <- rownames(variances_genes)
...
ggplot(variances_genes_R2X, aes(x=Dim, y=variance)) +
  geom_bar(stat = "identity") +
  theme_light() +
  labs(x = "principal components",
       y = "percentage of explained variance",
       title = "scree plot R2X")
...
Visualize the distribution of lipids levels, to ensure comparability across lipids and preparing for multivariate analyses.
